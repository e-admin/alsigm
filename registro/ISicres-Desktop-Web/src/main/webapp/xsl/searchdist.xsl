	...
	<HTML>
		...
		<BODY tabIndex="-1" style="cursor:default" onunload="top.EnableEvents(window.opener);">
			...
			<table class="report" width="99%">
				<xsl:apply-templates select="Sicreslist/DistFields"/>
			</table>
			<br/>
			<table class="report" width="99%">
			...
			<table id="tbButtons" width="99%" align="left" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
				<tr class="Style5" align="center" id="rowButtons">
					<td width="80%"/>
					<td width="10%">
						<input type="button" class="button" id="btnBuscar" name="btnBuscar" tabindex="1" STYLE="width:80" onclick="SearchDist();">
							<SCRIPT LANGUAGE="javascript">
	...
	<tr>
		...
		<td width="45%">
			<xsl:choose>
				...
				<xsl:otherwise>
					<input type="text" class="input" style="width:160" tabIndex="1" onblur="FormatField(this);">
						<xsl:attribute name="name"><xsl:value-of select='@Id'/></xsl:attribute>
						...
						<xsl:if test="@CaseSensitive[.='CS']">
							...
							<xsl:attribute name="onblur">this.value=this.value.toUpperCase();FormatField(this);</xsl:attribute>
						</xsl:if>
					</input>
					<xsl:if test="@Validation[.='1']">
						<IMG src="./images/buscar2.gif" tabIndex="1" style="cursor:pointer;position:relative;left:0;top:2">
							<xsl:attribute name="onclick">ShowValidationList("<xsl:value-of select='@Id'/>", <xsl:value-of select='@TValid'/>, <xsl:value-of select='@FldId'/>,'<xsl:value-of select="@CaseSensitive"/>');</xsl:attribute>
