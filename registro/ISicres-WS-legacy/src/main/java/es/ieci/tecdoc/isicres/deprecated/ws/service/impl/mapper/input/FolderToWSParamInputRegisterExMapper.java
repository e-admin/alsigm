...
public class FolderToWSParamInputRegisterExMapper implements Mapper {
	...
	private static final String NUMERO_REGISTRO_ORIGINAL_FIELD_ID="10";
	private static final String TIPO_REGISTRO_ORIGINAL_FIELD_ID="11";
	private static final String FECHA_REGISTRO_ORIGINAL_FIELD_ID="12";
	private static final String ENTIDAD_REGISTRAL_ORIGINAL_FIELD_ID="13";
	private static final String TIPO_TRANSPORTE_FIELD_ID="14";
	private static final String NUMERO_TRANSPORTE_FIELD_ID="15";
	private static final String TIPO_ASUNTO_FIELD_ID="16";

	private static final int MAX_FIELD_ID = 17;
	
	
	public Object map(Object obj) {
	
		Folder folder = (Folder)obj;
		WSParamInputRegisterEx register = new WSParamInputRegisterEx();
		...
		for (FieldInfo fieldInfo : listaFields) {
			...
			else if(NUMERO_REGISTRO_ORIGINAL_FIELD_ID.equals(fieldId))
			{
				register.setOriginalNumber(fieldValue);
			}
			else if(TIPO_REGISTRO_ORIGINAL_FIELD_ID.equals(fieldId))
			{
				register.setOriginalType(Integer.valueOf(fieldValue));
			}
			else if(FECHA_REGISTRO_ORIGINAL_FIELD_ID.equals(fieldId))
			{
				...
				try
				{
					register.setOriginalDate(XMLGregorianCalendarHelper.toXMLGregorianCalendar(longFormatter.parse(fieldValue)));
				}
				catch (Exception e) {
					logger.error("No se puede parsear la fecha original.", e);
					throw new RuntimeException("No se puede parsear la fecha original.");
				}
			}
			else if(ENTIDAD_REGISTRAL_ORIGINAL_FIELD_ID.equals(fieldId))
			{
				register.setOriginalEntity(fieldValue);
			}
