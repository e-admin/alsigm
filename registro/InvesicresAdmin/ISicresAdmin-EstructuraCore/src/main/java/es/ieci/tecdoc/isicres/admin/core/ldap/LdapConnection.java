...
public final class LdapConnection {
	 ...
	 * @throws Exception
	 */
	public void open(int engine, int provider, String url, String userDn,
			String userPwd, boolean pool, int poolTimeout) throws Exception {
		open(engine, provider, url, userDn, userPwd, pool, poolTimeout, LDAP_VERSION_2);
	}


	/**
	 * Abre la conexión ldap especificando la versión del ldap: "2" o "3".
	 *
	 * engine: ActiveDirectory, iPlanet, ...
	 *
	 * provider: Sun, ...
	 ...
	 */
	public void open(int engine, int provider, String url, String userDn,
			String userPwd, boolean pool, int poolTimeout, String ldapVersion)
			throws Exception {
	...
	/**
	 * Codifica la cadena de conexión al ldap.
	 *
	 * Los CN y DN configurados podrían tener espacios en blanco, caracteres que
	 * no son válidos según la especificación de URLS (RFC 1738).
	 *
	 * http://docs.oracle.com/javase/1.5.0/docs/guide/jndi/jndi-ldap-gl.html
	 *
	 * @param url
	...
	public void open(LdapConnCfg cfg) throws Exception {
		...
		if (StringUtils.isBlank(cfg.getLdapVersion())){
			open(cfg.getEngine(), cfg.getProvider(), cfg.getUrl(), cfg.getUser(),
				cfg.getPwd(), cfg.getPool(), cfg.getPoolTimeOut());
		}else{
			open(cfg.getEngine(), cfg.getProvider(), cfg.getUrl(), cfg.getUser(),
					cfg.getPwd(), cfg.getPool(), cfg.getPoolTimeOut(), cfg.getLdapVersion());
		}
	...
	private static void checkEngine(int engine) throws Exception {
		...
		if (engine == LdapEngine.ACTIVE_DIRECTORY)
			return;
		if (engine == LdapEngine.I_PLANET)
			return;
		if (engine == LdapEngine.OPENLDAP)
			return;
