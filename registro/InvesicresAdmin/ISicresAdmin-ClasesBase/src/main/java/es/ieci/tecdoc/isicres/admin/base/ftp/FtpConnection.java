package es.ieci.tecdoc.isicres.admin.base.ftp;
...
public final class FtpConnection
{
   ...
   public void open(String host, int port, String user, String pwd)
      throws IeciTdException, Exception
   {
	  // En Windows, si se intenta hacer ftp en la ip 127.0.0.1 o nombre de máquina localhost,
	  // al crear el socket de datos para enviar un fichero se produce un error de "500 Illegal PORT Command".
	  // En Unix esto no sucede
	  /*
	  if (host.equals("127.0.0.1") || host.equalsIgnoreCase("localhost")) {
		  host = FtpCoreFns.getLocalHostAddress();
	  }
	  */
      ...
      throws IeciTdException, Exception
   {
      ...
      if (!isPcResponse())
      {
         ...
         if (!isPcResponse()) { 
        	 throw new IeciTdException(FtpError.EC_NATIVE_ERR, m_respText.toString());
         }
