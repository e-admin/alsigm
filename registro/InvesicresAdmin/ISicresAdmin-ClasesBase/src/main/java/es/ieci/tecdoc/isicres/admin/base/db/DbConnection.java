...
// ---------------------------------------------
   ...
   private static String  m_engineVersion;    
	...
	public static synchronized Connection open() throws Exception
	{
		...
		if (DbConfig.isDataSource())
		{
			jdbcConn = open(DbConfig.getDataSource());
		}
		else
		{
			jdbcConn = open(DbConfig.getDriver(), DbConfig.getUrl(), DbConfig
                      ...
                      throws Exception
   {
      ...
      /* NO NOS INTERESA QUE SI EXISTE UNA TRANSACCION DE ERROR
   ...
   public static boolean beginTransactionExt()
                      throws Exception
   {
