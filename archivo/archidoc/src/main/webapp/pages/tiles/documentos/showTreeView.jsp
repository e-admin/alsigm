...
<head>
	...
	<script type="text/javascript">
		function refreshWindow(selectedNode) {
			refreshDocumentTreeWindow(selectedNode);
		}
		
		function refreshDocumentTreeWindow(selectedNode) {
...
<tree:show action="documentos" name="<%=viewName%>" imageConfFile="/js/documentos_tree_tpl.js" target="basefrm">
	function getNodeImage(treeItem) {
		var customIcon = null;
		if (treeItem.o_root.a_tpl['imgs_icons'] && treeItem.o_root.a_tpl['imgs_icons'][treeItem.a_config[1].TIPO_NODO]) {
			customIcon = treeItem.o_root.a_tpl['imgs_icons'][treeItem.a_config[1].TIPO_NODO][(treeItem.a_config[3]?0:4)+(treeItem.a_config[3]&&treeItem.b_opened?2:0)+(treeItem.selected?1:0)];
		}
		return customIcon;
	}
</tree:show>
