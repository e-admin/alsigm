...
<c:set var="listaUdocs" value="${sessionScope[appConstants.consultas.LISTADO_BUSQUEDA_UDOCS]}" />

<bean:struts id="mappingGestionDetallePrestamo" mapping="/gestionDetallesConsultas" />
		...
		<c:if test="${!empty listaUdocs}">
			...
			<c:url var="paginationURI" value="/action/gestionDetallesConsultas">
			...
			<display:table name="pageScope.listaUdocs"
				...
				<c:if test="${beanBusqueda.mapSalida[appConstants.prestamos.camposBusquedas.CAMPO_SALIDA_BUSQUEDA_RANGOS_NUMERO_EXPEDIENTE] != null}">
					<display:column titleKey="archigest.archivo.prestamos.expedienteudoc" style="width:18%;" media="html">
						<c:choose>
							<c:when test="${udocs.subtipoCaja}">
					...
					<display:column titleKey="archigest.archivo.prestamos.expedienteudoc" style="width:18%;"  media="csv excel pdf xml">
						<c:choose>
							<c:when test="${udocs.subtipoCaja}">
								<bean:message key="archigest.archivo.simbolo.rango"/> <c:out value="${udocs.nombreRangos}"/>
							</c:when>
							<c:otherwise>
								<c:out value="${udocs.numexp}"/>
							</c:otherwise>
				...
				<c:if test="${requestScope[appConstants.consultas.VER_BOTONES_COPIAS]}">
					<display:column titleKey="archigest.archivo.solicitudes.copiasSimples">
					<c:choose>
						<c:when test="${udocs.subtipoCaja}">
							<input type="text" size="1" name= "copiaSimple|<c:out value="${udocs.identificacion}"/>|<c:out value="${udocs.id}"/>|null|<c:out value="${udocs.titulo}"/>|<c:out value="${udocs.signaturaudoc}"/>|<c:out value="${udocs.idFondo}"/>|<c:out value="${udocs.codsistproductor}">null</c:out>" class="input">
						</c:when>
						<c:otherwise>
							<input type="text" size="1" name="copiaSimple|<c:out value="${udocs.identificacion}"/>|<c:out value="${udocs.id}"/>|<c:out value="${udocs.numexp}">null</c:out>|<c:out value="${udocs.titulo}"/>|<c:out value="${udocs.signaturaudoc}"/>|<c:out value="${udocs.idFondo}"/>|<c:out value="${udocs.codsistproductor}">null</c:out>" class="input">
						</c:otherwise>
					</c:choose>

					</display:column>
					<display:column titleKey="archigest.archivo.solicitudes.copiasCertificadas">
						<c:choose>
							<c:when test="${udocs.subtipoCaja}">
								<input type="text" size="1"  name="copiaCertificada|<c:out value="${udocs.identificacion}"/>|<c:out value="${udocs.id}"/>|null|<c:out value="${udocs.titulo}"/>|<c:out value="${udocs.signaturaudoc}"/>|<c:out value="${udocs.idFondo}"/>|<c:out value="${udocs.codsistproductor}">null</c:out>" class="input">
							</c:when>
							<c:otherwise>
								<input type="text" size="1" name="copiaCertificada|<c:out value="${udocs.identificacion}"/>|<c:out value="${udocs.id}"/>|<c:out value="${udocs.numexp}">null</c:out>|<c:out value="${udocs.titulo}"/>|<c:out value="${udocs.signaturaudoc}"/>|<c:out value="${udocs.idFondo}"/>|<c:out value="${udocs.codsistproductor}">null</c:out>" class="input">
							</c:otherwise>
						</c:choose>
