...
<c:set var="detalles" value="${sessionScope[appConstants.consultas.DETALLE_CONSULTA_KEY]}"/>
	...
	<tiles:put name="boxContent" direct="true">
		...
		<tiles:insert page="/pages/tiles/PADockableBlockLayout.jsp">
			...
			<tiles:put name="dockableContent" direct="true">
				<tiles:insert page="/pages/tiles/solicitudes/consultas/datos_consulta.jsp" />
			</tiles:put>
		</tiles:insert>		
		...
		<tiles:insert page="/pages/tiles/PABlockLayout.jsp">
			<tiles:put name="blockTitle" direct="true">
				<bean:message key="archigest.archivo.consultas.udocs"/>
			</tiles:put>
			<tiles:put name="blockContent" direct="true">
					
				<form name="form0" method="post" action="<c:url value="/action/papeletaDevolucion"/>">
				<input type="hidden" name="idConsulta" value="<c:out value="${bConsulta.id}"/>"/>
				...
				<display:table name="pageScope.detalles"
					...
					sort="list"
					pagesize="10"
					requestURI="/action/gestionConsultas">
					<display:setProperty name="basic.msg.empty_list">
						<bean:message key="archigest.archivo.consultas.noDetallesConsultas"/> 
					</display:setProperty>
	
					<display:column style="width:20px;">
						<c:if test="${detalleConsulta.estado==appConstants.consultas.ESTADO_DETALLE_DEVUELTA}">
							<input type="checkbox" name="idUdoc" value="<c:out value="${detalleConsulta.idudoc}.${detalleConsulta.signaturaudoc}" />">
						</c:if>
					</display:column>
						...
						<c:choose>
							<c:when test="${detalleConsulta.subtipoCaja}">
								...
								<c:if test="${!empty detalleConsulta.nombreRangos}">
									<c:out value="${detalleConsulta.nombreRangos}"/><br/>
								</c:if>
