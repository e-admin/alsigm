...
<bean:struts id="mappingGestionCLSeries" mapping="/gestionClasificadorSeriesAction" />
<c:set var="clserieFormBean" value="${requestScope[mappingGestionCLSeries.name]}" />
...
<tiles:insert template="/pages/tiles/PABoxLayout.jsp">
	...
	<tiles:put name="buttonBar" direct="true">
		...
		<TR>
			<TD>
				<a class="etiquetaAzul12Bold" href="javascript:document.forms['<c:out value="${mappingGestionCLSeries.name}" />'].submit()" >
	...
	<tiles:put name="boxContent" direct="true">
		<html:form action="/gestionClasificadorSeriesAction">
			<input type="hidden" name="method" value="guardarInfoCLSerie">
							...
							<TD colspan="3">
								<TABLE class="formulario" cellpadding="0" cellspacing="0" width="100%">
									<tr>
										...
										<td class="tdDatos">
											<script>
												...
												{
													var form = document.forms["<c:out value="${mappingGestionCLSeries.name}" />"];
									...
									<tr id="trListaControlAcceso">
										...
										<td class="tdDatos">
											<select style="width:100%;" name="idLCA" <c:if test="${ClasificadorSerieForm.nivelAcceso == 1}">disabled="true"</c:if>>
												<option value="">&nbsp;</option>
												<c:forEach var="lca" items="${listasControlAcceso}">
													<option value="<c:out value="${lca.id}"/>"
														<c:if test="${lca.id==ClasificadorSerieForm.idLCA}">selected="true"</c:if>
														>
