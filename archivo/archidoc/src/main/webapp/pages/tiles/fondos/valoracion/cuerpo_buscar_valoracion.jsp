...
<bean:struts id="mappingBuscarValoracion" mapping="/buscarValoracion" />

<script language="JavaScript1.2" src="<%=request.getContextPath()%>/js/utils.js" type="text/JavaScript"></script>
<script language="JavaScript1.2" type="text/JavaScript">
	function clean()
	{
		document.forms['<c:out value="${mappingBuscarValoracion.name}" />'].fondo.value = "";
		document.forms['<c:out value="${mappingBuscarValoracion.name}" />'].codigo.value = "";
		document.forms['<c:out value="${mappingBuscarValoracion.name}" />'].tituloSerie.value = "";
		document.forms['<c:out value="${mappingBuscarValoracion.name}" />'].tituloValoracion.value = "";
	...
	function buscar(){
		if (window.top.showWorkingDiv) {
			...
			var message = '<bean:message key="archigest.archivo.buscando.msgValoraciones"/>';
			var message2 = '<bean:message key="archigest.archivo.msgOperacionLenta"/>';
			window.top.showWorkingDiv(title, message, message2);
		}
		document.forms['<c:out value="${mappingBuscarValoracion.name}" />'].submit();
	}

</script>

<html:form action="/buscarValoracion?method=buscar" >
<tiles:insert template="/pages/tiles/PABoxLayout.jsp">
	...
	<tiles:put name="boxContent" direct="true">
		...
		<table class="formulario">					
			...
			<tr>
				<td class="tdTitulo"><bean:message key="archigest.archivo.valoracion.buscar.codigo"/>:&nbsp;</td>
				<td class="tdDatos"><input type="text" size="30" name="codigo" value="<c:out value="${param.codigo}" />"></td>
			</tr>
			<tr>
				<td class="tdTitulo"><bean:message key="archigest.archivo.valoracion.buscar.tituloSerie"/>:&nbsp;</td>
