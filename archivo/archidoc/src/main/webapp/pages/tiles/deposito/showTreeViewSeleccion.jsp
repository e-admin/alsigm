...
<body>
	...
	<c:set var="viewName"><c:out value="${param.viewName}"><c:out value="${appConstants.deposito.DEPOSITO_SEL_VIEW_NAME}" /></c:out></c:set>
	<jsp:useBean id="viewName" type="java.lang.String" />

	<tree:show name="<%=viewName%>" action="/depositoSeleccion" target="basefrm">
		...
		function viewNodeContent(tree_id, node_id)
		{
			if (parent.setUbicacion)
			{
				...
				if (tipoElem && Number(tipoElem) == 1)
					parent.setUbicacion(idElem, nombre);
				else
					alert("<bean:message key="archigest.archivo.deposito.busqueda.ubicacion.invalida"/>");
			}
		}
	</tree:show>
