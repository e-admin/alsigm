...
<body>
	...
	<c:set var="viewName"><c:out value="${param.viewName}"><c:out value="${appConstants.deposito.DEPOSITO_SEL_SUBVIEW_NAME}" /></c:out></c:set>
		...
		function viewNodeContent(tree_id, node_id) 
		{
			if (parent.setAmbito)
			{
				...
				if (tipoElem && Number(tipoElem) > 1)
					parent.setAmbito(idElem, pathElem,codOrdenElem);
				else
