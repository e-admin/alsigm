...
<c:set var="descriptores" value="${requestScope[appConstants.descripcion.LISTA_PRODUCTORES_RELACION]}"/>
...
<script language="JavaScript1.2" type="text/JavaScript">
	...
	function selectList()
	{
		...
		if (selectDescriptores)
		{
			...
			for (var i = 0; i < selectElem.options.length; i++)
			{
				...
				if ((field!=null)&&(field.value == 'productoresSeleccionados')){
	...
	function search()
	{
		...
		document.forms["<c:out value="${actionMapping.name}" />"].method.value = "searchProductoresRelacion";
...
<div id="contenedor_ficha_centrada">
  ...
  <input type="hidden" name="method" value="searchProductoresRelacion"/>
				   ...
				   <c:if test="${descriptores != null}">
					...
					<display:table name="pageScope.descriptores"
						...
						<display:setProperty name="basic.msg.empty_list">
						  <bean:message key="archigest.archivo.descripcion.productor.vacio"/>
						</display:setProperty>
