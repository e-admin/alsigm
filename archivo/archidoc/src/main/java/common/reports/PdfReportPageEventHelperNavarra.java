 ...
 */
public class PdfReportPageEventHelperNavarra extends PdfReportPageEventHelper {
	 ...
	 * Constructor.
	 */
	public PdfReportPageEventHelperNavarra(String entity) {
		super(entity);
	}
	...
	protected void drawHeader(PdfWriter writer, Document document) {
		try {
			...
			tableInt.setTotalWidth(new float[] { 380,
					document.right() - document.left() - 380 });
			...
			tableInfo.addCell(new Phrase(new Chunk(" ", CABECERA_H1_FONT)));
			tableInfo.addCell(new Phrase(new Chunk(TITULO, CABECERA_H1_FONT)));
			tableInfo
					.addCell(new Phrase(new Chunk(DIRECCION, CABECERA_H2_FONT)));
