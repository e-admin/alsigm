...
public class ActionRedirect extends ActionForward {
	...
	public void addParameter(String fieldName, Object valueObj) {
		addParameter(fieldName, valueObj, true);
	}

	public void addParameter(String fieldName, Object valueObj, boolean encode) {
		...
		if (encode)
			value = StringUtils.encodeUrlParameterValue(value);
		...
		if (currentValue == null) {
			...
			List newValues = new ArrayList(
					Arrays.asList((Object[]) currentValue));
			newValues.add(value);
	...
	public String getPath() {
		return RequestUtil.getPath(super.getPath(), parameterValues);
	}

	/*
	 * public String getParameterString() { StringBuffer strParam = new
	 * StringBuffer(256);
	 * 
	 * // loop through all parameters Iterator iterator =
	 * parameterValues.keySet().iterator(); while (iterator.hasNext()) { // get
	 * the parameter name String name = (String) iterator.next();
	 * 
	 * // get the value for this parameter Object value =
	 * parameterValues.get(name);
	 * 
	 * if (value instanceof String) { // just one value for this param
	 * strParam.append(name) .append("=") .append(value);
	 * 
	 * } else if (value instanceof String[]) { // loop through all values for
	 * this param String[] values = (String[]) value; for (int i = 0; i <
	 * values.length; i++) { strParam.append(name) .append("=")
	 * .append(values[i]); if (i < values.length - 1) strParam.append("&"); } }
	 * 
	 * if (iterator.hasNext()) { strParam.append("&"); } }
	 * 
