...
public class BusquedaTareaForm extends CustomForm {
	/**
	 *
	 */
	...
	public BusquedaTareasVO getBusquedaVO() {
		BusquedaTareasVO vo = new BusquedaTareasVO();
		...
		if (estados.length == 0)
			errors.add(
					...
					new ActionError(
							...
							Messages.getString(
									DocumentosConstants.LABEL_DIGITALIZACION_BUSQUEDA_ESTADO,
									request.getLocale())));
		...
		if (!new CustomDate(this.fechaFinFormato, this.fechaA, this.fechaM,
			...
			errors.add(
					...
					new ActionError(
							...
							Messages.getString(
									DocumentosConstants.LABEL_DIGITALIZACION_BUSQUEDA_FECHA,
									request.getLocale())));
		}
