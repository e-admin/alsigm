CREATE VIEW <username>.SPAC_WL_BATCHTASK AS 
 SELECT <username>.SPAC_TRAMITACIONES_AGRUPADAS.ID AS ID_AGRUPACION, <username>.SPAC_TRAMITACIONES_AGRUPADAS.ID_RESP, <username>.SPAC_TRAMITACIONES_AGRUPADAS.ESTADO, <username>.SPAC_TRAMITACIONES_AGRUPADAS.FECHA_CREACION, <username>.SPAC_P_FASES.NOMBRE AS FASE, <username>.SPAC_TRAMITACIONES_AGRUPADAS.ID_ULTIMO_TRAMITE, <username>.SPAC_TRAMITACIONES_AGRUPADAS.ID_ULTIMO_TEMPLATE, <username>.SPAC_TRAMITACIONES_AGRUPADAS.ID_ULTIMO_TIPODOC, <username>.SPAC_CT_PROCEDIMIENTOS.COD_PCD, <username>.SPAC_CT_PROCEDIMIENTOS.NOMBRE AS PROCEDIMIENTO, <username>.SPAC_TRAMITACIONES_AGRUPADAS.ID_FASE
   FROM <username>.SPAC_TRAMITACIONES_AGRUPADAS, <username>.SPAC_P_FASES, <username>.SPAC_CT_PROCEDIMIENTOS
  WHERE <username>.SPAC_TRAMITACIONES_AGRUPADAS.ID_FASE = <username>.SPAC_P_FASES.ID AND <username>.SPAC_P_FASES.ID_PCD = <username>.SPAC_CT_PROCEDIMIENTOS.ID

GO
    
    
CREATE VIEW <username>.SPAC_WL_PROC AS 
	SELECT FASES.ID_EXP AS ID, FASES.NUMEXP, FASES.ID_PCD, FASES.ID_FASE AS ID_STAGEPCD, FASES.ID AS ID_STAGE, FASESPCD.NOMBRE AS NAME_STAGE, FASES.FECHA_INICIO AS INITDATE, FASES.FECHA_LIMITE AS LIMITDATE, FASES.ID_RESP AS RESP, FASES.TIPO FROM <username>.SPAC_FASES FASES, <username>.SPAC_P_FASES FASESPCD WHERE FASES.ID_FASE = FASESPCD.ID AND FASES.ESTADO = 1
GO

CREATE VIEW <username>.SPAC_WL_PCD AS
SELECT
...
CREATE VIEW <username>.SPAC_WL_TASK  AS
...
CREATE VIEW <username>.SPAC_WL_ACTIVITY AS 
