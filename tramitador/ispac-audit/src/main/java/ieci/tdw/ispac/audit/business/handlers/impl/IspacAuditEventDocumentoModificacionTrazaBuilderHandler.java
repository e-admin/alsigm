 ...
 * 
 */
public class IspacAuditEventDocumentoModificacionTrazaBuilderHandler implements IspacAuditoriaEventHandler {
	...
	public Object handle(IspacAuditEventVO ispacAuditEvent) {
		...
		IspacAuditEventDocumentoModificacionVO evento = (IspacAuditEventDocumentoModificacionVO) ispacAuditEvent;
	...
	protected TrazaAuditoriaVO generateTraza(IspacAuditEventDocumentoModificacionVO evento) {
		...
		result.setEventType(new Long(IspacEventAuditTypeEnum.DOCUMENTO_MODIFICACION.getValue()));

		// descripción del evento que se produjo con info adicional
		String eventDescription = "Documento - MODIFICACIÓN";
		...
		result.setNewValue(evento.getNewValue());

		return result;
	}

	protected void populateBasicUserEventTrazaAuditoria(
			IspacAuditEventDocumentoModificacionVO evento, TrazaAuditoriaVO traza) {
