...
public class ShowClosedTasksListAction extends BaseAction {
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (request.getParameter("taskIds")!=null){
			...
			} else if (taskCtlId != 0) { //Si existe el identificador del trámite en el catálogo...
				...
				ClosedTaskListFormatFactory tasklistfactory = new ClosedTaskListFormatFactory(getISPACPath(), taskType);
				formatter = tasklistfactory.getBeanFormatter(taskCode, taskType);
				
				// Identificador del procedimiento
				String pcdId = request.getParameter("pcdId");
				if (StringUtils.isNotBlank(pcdId)) {
					itcTask = workListAPI.getClosedTasksCTL(taskCtlId, TypeConverter.parseInt(pcdId, -1));
				} else {				
					itcTask = workListAPI.getClosedTasksCTL(taskCtlId);
				}
				
			} else if (taksPcdId != 0) { //Si existe el identificador del trámite para el procedimiento...
			    
				//...se obtienen todos los tramites abiertos para un tipo de 
				//procedimiento que segun este identificador
			    itcTask = workListAPI.getClosedTasksPCD(newState.getTaskPcdId());
			}
