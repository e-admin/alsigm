...
public class ServicioTramitacionRemoteClientTest extends TestCase {
	...
	private static final String TRAMITACION_WSDL_URL = "http://localhost:8080/ispac-services-ws-war/services/TramitacionWebService?wsdl";
	...
	public void testProcedimientosPorTipo(){
		...
		try {
			...
			InfoBProcedimiento[] procs = servicio.getProcedimientos(
					InfoBProcedimiento.ALL_PROCEDURES, null);
			log(procs);

			log("b) Tipo ONLY_AUTOMATIZED:");
			procs = servicio.getProcedimientos(
	...
	public void testProcedimientos(){
		...
		try {
			...
			log("a) ids={}:");
			InfoBProcedimiento[] procs = servicio.getProcedimientos(new String[0]);
			log(procs);

			log("b) ids={'PCD-3','PCD-4'}:");
			procs = servicio.getProcedimientos(new String[] { "PCD-3", "PCD-4" });
			log(procs);

			log("c) ids={'PCD-555','PCD-3'}:");
			procs = servicio.getProcedimientos(new String[] { "PCD-555", "PCD-3" });
			log(procs);
	...
	public void testProcedimiento(){
		...
		try {
			...
			Procedimiento proc = servicio.getProcedimiento("PCD-3");
			log(proc);
	...
	public void testFichero(){
		...
		try {
			...
			byte[] contenido = servicio.getFichero( "<guid><archive>4</archive><folder>70</folder><document>1</document></guid>");
	...
	public void testInfoFichero(){
		...
		try {
			...
			InfoFichero info = servicio.getInfoFichero("<guid><archive>4</archive><folder>68</folder><document>1</document></guid>");
			log(info);
	...
	public void testInfoOcupacion(){
		...
		try {
			...
			InfoOcupacion info = servicio.getInfoOcupacion();
			log(info);
	...
	public void testEliminarFicheros(){
		...
		try {
			...
			servicio.eliminaFicheros( new String[] {
					"<guid><archive>3</archive><folder>802</folder><document>15</document></guid>"
			});
	...
	public void testIdsExpedientes(){
		...
		try {
			...
			cal.set(1900, 0, 1);
			String[] ids = servicio.getIdsExpedientes( "PCD-3", cal.getTime(), new Date(), 0);
			log(ids);
	...
	public void testExpedientes(){
		...
		try {
			...
			InfoBExpediente[] exps = servicio.getExpedientes( new String[] {"EXP/266"});
			log(exps);
	...
	public void testExpediente(){
		...
		try {
			...
			Expediente exp = servicio.getExpediente( "EXP/266");
			log(exp);
	...
	public void testIniciarExpediente(){
		...
		try {
			...
			boolean creado = servicio.iniciarExpediente( datosComunes, 
	...
	public void testMoverExpedienteAFase(){
		...
		try {
			ServicioTramitacion servicio = getServicioTramitacion();
			String numExp = "2009/000843";
			//Fase de Instruccion
			String idFaseCatalogo = "2";
			
			boolean resultado = servicio.moverExpedienteAFase(numExp, idFaseCatalogo);
			if (resultado){
	...
	public void testCambiarEstadoAdministrativo(){
		...
		try {		
			ServicioTramitacion servicio = getServicioTramitacion();	
			String numExp = "2009/000845";
			String estadoAdm = "PA";
			servicio.cambiarEstadoAdministrativo(numExp,estadoAdm );
	...
	public void testAnexarDocsExpediente(){
		...
		try {
			...
			boolean anexados = servicio.anexarDocsExpediente( 
					"EXPAYTOVI2008/1620",
					"20070001", 
					new Date(), 
			...
			anexados = servicio.anexarDocsExpediente("EXPAYTOVI1/6",
					"20070001", 
					new Date(), 
					(DocumentoExpediente[])documentos.toArray(
							new DocumentoExpediente[documentos.size()]));
	...
	public void testArchivarExpedientes(){
		...
		try {
			...
			String[] exps = servicio.getIdsExpedientes("PCD-3", fechaIni,
					fechaFin, 1); // Ordenamos por numero de expediente

			// Archivo el primer expediente
			if (exps != null && exps.length > 0) {
				servicio.archivarExpedientes(new String[] { exps[0] });
			}

			// Compruebo q tenemos los mismo expediente de antes salvo el
			// primero
			exps = servicio.getIdsExpedientes("PCD-3", fechaIni, fechaFin, 1);
	...
	public void testBusquedaAvanzada(){
		log("\n\n--- Entrada en testBusquedaAvanzada ---");
		...
		try {
			...
			//Con porcentaje de discapacidad
			resultado = servicio.busquedaAvanzada(groupName, searchFormName, searchXML, domain);
			log("Resultado de busqueda: " + resultado);		

			
		} catch (Exception e) {
			fail("Error: " + e.toString());
			log("Error en testBusquedaAvanzada", e);
		}
		
		log("--- Salida de testBusquedaAvanzada ---");
	...
	public void testEstablecerRegistroEntidad(){
		...
		try {
			...
			resultado = servicio.establecerDatosRegistroEntidad(entidadTramitacion, numExp, datos);
			log("Identificador del registro creado: " + resultado);		

			
			datosRegistro = servicio.obtenerRegistroEntidad(entidadTramitacion, numExp, resultado);
			...
			datos = "<?xml version=\"1.0\" encoding=\"iso-8859-1\" ?><fichaSeguimiento><id>0</id><observaciones></observaciones></fichaSeguimiento>";
			resultado = servicio.establecerDatosRegistroEntidad(entidadTramitacion, numExp, datos);

			datosRegistro = servicio.obtenerRegistroEntidad(entidadTramitacion, numExp, 0);
			log("Datos registro : " + datosRegistro);				
	...
	public void testObtenerRegistroEntidad(){
		...
		try {
			...
			String datosRegistro = servicio.obtenerRegistroEntidad(entidadTramitacion, numExp, 29);
			log("Datos registro : " + datosRegistro);					
	...
	public void testObtenerRegistrosEntidad(){
		...
		try {
			...
			String datosRegistro = servicio.obtenerRegistrosEntidad(entidadTramitacion, numExp);
