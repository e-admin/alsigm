			...
			<div id="bodytile" style="visibility:visible;">
				...
				<c:choose>
					...
					<c:when test="${!empty wizardSubtitleKey}">
						...
						<c:set var="subtitle">
							<bean:message key='<%= String.valueOf(wizardSubtitleKey)%>'/> '<bean:write scope="session" name="entityBean" property="property(SPAC_CT_ENTIDADES:NOMBRE)"/>'
						</c:set>
					</c:when>
					<c:otherwise>
				...
				--%>
				<div id="navmenu">
					<c:set var="labelsList" value="${sessionScope['labelsListKey']}"/>
					<c:if test="${!empty labelsList}">
					<ul class="actionsMenuList">
						<c:forEach var="label" items="${labelsList}">
						<c:set var="titleKey" value="${label.titleKey}"/>
						<bean:define id="titleKey" name="titleKey" type="java.lang.String"/>
						<c:choose>
							<c:when test="${label.active}">
								<li>
									<a href='<c:out value="${label.url}" escapeXml="false"/>'><nobr><bean:message key='<%=titleKey%>'/></nobr></a>
								</li>
							</c:when>
							<c:otherwise>
