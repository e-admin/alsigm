...
<script>
	function submitPermission(myform)
	{
		...
		{
			jAlert('<bean:message key="catalog.report.permission.alert.noResponsibles"/>', '<bean:message key="common.alert"/>', '<bean:message key="common.message.ok"/>' , '<bean:message key="common.message.cancel"/>');
...
<c:url value="showReport.do" var="_return">
	<c:param name="view">
...
<c:url value="/entrySearch.do" var="_search">
	...
	<c:param name="actionSetResponsibles" value="/addResponsiblesReport.do"/>

</c:url>

<c:url value="${actionShow}" var="_showResponsiblesForReport">
	<c:param name="view">
		<bean:write name="view"/>
	</c:param>
	<c:param name="id">
...
<div class="encabezado_ficha">
	<div class="titulo_ficha">
	  <h4><bean:message key="catalog.report.permission.selectResponsibles"/></h4>
...
<div id="navmenu">
	<ul class="actionsMenuList">
		...
		<li><a href='<%=_showResponsiblesForReport + "&supervDirView=organization"%>' id="current"><bean:message key="catalog.report.organization"/></a></li>
		<li><a href='<%=_showResponsiblesForReport + "&supervDirView=groups"%>'><bean:message key="catalog.report.groups"/></a></li>
...
<table cellpadding="0" cellspacing="0" width="95%" align="center">
	...
	<c:otherwise>
		<c:set var="actionAdd" value="addResponsiblesReport.do"/>
	</c:otherwise>
	</c:choose>
	
	<c:url value="${actionAdd}" var="_addResponsiblesReport">
		<c:param name="view">
			<bean:write name="view"/>
		</c:param>
	...
	<jsp:useBean id="_addResponsiblesReport" type="java.lang.String"/>
					...
					<tr>
						<td>
												...
												<logic:iterate id="format" name="SupervisionFormatter" type="ieci.tdw.ispac.ispaclib.bean.BeanPropertyFmt">
													<logic:equal name="format" property="property" value="RESPNAME">
														...
														<logic:notEqual name="cnt" value="0">
															<%out.print(',');%>&nbsp;&nbsp;
															<%
																String orgUrl2 = _showResponsiblesForReport + "&supervDirView=organization";
							...
							<div class="stdform">
								...
								<html:form action='<%=_addResponsiblesReport + "&supervDirView=organization&dirUid=" + dirUid%>'>
											...
											<tr>
												...
												<td align="left" class="ldapentry" title='<%= orgunit.getProperty("RESPNAME") %>'>
													<logic:iterate id="format" name="SupervisionFormatter" type="ieci.tdw.ispac.ispaclib.bean.BeanPropertyFmt">
														<logic:equal name="format" property="property" value="RESPNAME">
															<%
																String orgURL3 = _showResponsiblesForReport + "&supervDirView=organization";
					...
					<tr>
						<td>
							...
							<div class="stdform">
								...
								<html:form action='<%=_addResponsiblesReport + "&supervDirView=groups"%>'>
												...
												<tr>
													...
													<td colspan='2' align='left' class='ldapentry' title='<%= group.getProperty("RESPNAME") %>'>
														<logic:iterate id="format" name="SupervisionFormatter" type="ieci.tdw.ispac.ispaclib.bean.BeanPropertyFmt">
															<logic:equal name="format" property="property" value="RESPNAME">
																<%
																	String grp = _showResponsiblesForReport + "&supervDirView=groups";
