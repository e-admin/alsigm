...
<script>
	function submitSuperviseds(myform)
	{
    	var checked = false;
...
<c:url value="/entrySearch.do" var="_search">
	...
	<logic:present name="regId">
		<c:param name="regId">
			<bean:write name="regId" />
		</c:param>
	</logic:present>
	<logic:present name="captionkey">
		<c:param name="captionkey">
	...
	<logic:present name="kindOfSuperv">
		...
		<c:param name="actionSetResponsibles" value="/addSuperviseds.do"/>
	</logic:present>
	<logic:notPresent name="kindOfSuperv">
	<c:param name="actionSetResponsibles" value="/gestionSustituciones.do?method=addSubstitutes"/>
	</logic:notPresent>
...
<div class="encabezado_ficha">
	...
	<div class="acciones_ficha">
		...
		<a  class="btnOk" href='javascript:submitSuperviseds(document.<%=formName%>);'><bean:message key="forms.button.accept"/></a>
...
<div id="navmenu">
	<ul class="actionsMenuList">
	    <li><a href='javascript:submitSuperviseds(document.<%=formName%>);'><bean:message key="forms.button.accept"/></a></li>
		<li><a href='javascript:parent.hideFrame("workframe","<ispac:rewrite page="wait.jsp"/>")'>
				<bean:message key="forms.button.cancel"/>
			</a>
		</li>
	</ul>
...
<div id="navmenu">
	<ul class="actionsMenuList">
		...
		<li><a href='<%=_showResponsiblesForSupervision + "&supervDirView=organization"%>' id="current"><bean:message key="catalog.supervision.organization"/></a></li>
		<li><a href='<%=_showResponsiblesForSupervision + "&supervDirView=groups"%>'><bean:message key="catalog.supervision.groups"/></a></li>
...
<table cellpadding="0" cellspacing="0" width="95%" align="center">
	...
	<c:choose>
	<c:when test="${!empty action}">
		<c:set var="actionAdd" value="${action}?method=addSubstitutes"/>
	</c:when>
	<c:otherwise>
		<c:set var="actionAdd" value="addSuperviseds.do"/>
	</c:otherwise>
	</c:choose>
	
	<c:url value="${actionAdd}" var="_addSuperviseds">
		<c:param name="view">
			<bean:write name="view"/>
		</c:param>
		<c:param name="uid">
	...
	<jsp:useBean id="_addSuperviseds" type="java.lang.String"/>
					...
					<tr>
						<td>
												...
												<logic:iterate id="format" name="SupervisionFormatter" type="ieci.tdw.ispac.ispaclib.bean.BeanPropertyFmt">
													<logic:equal name="format" property="property" value="RESPNAME">
														...
														<logic:notEqual name="cnt" value="0">
															<%out.print(',');%>&nbsp;&nbsp;
															<%
																String orgUrl2 = _showResponsiblesForSupervision + "&supervDirView=organization";
							...
							<div class="stdform">
								...
								<html:form action='<%=_addSuperviseds + "&supervDirView=organization&dirUid=" + dirUid%>'>
									...
									<table class="tableDisplayUserManager">
										...
										<logic:iterate id="user" name="supervUsers" type="ieci.tdw.ispac.ispaclib.bean.ItemBean">
											<tr>
												<td width='17px'>
													<logic:empty name="user" property="property(SELECTED)">
										...
										<logic:iterate id="orgunit" name="supervOrgunits" type="ieci.tdw.ispac.ispaclib.bean.ItemBean">
											<tr>
												<td width='17px'>
													<logic:empty name="orgunit" property="property(SELECTED)">
														<html:multibox property="uids">
												...
												<td align="left" class="ldapentry" title='<%= orgunit.getProperty("RESPNAME") %>'>
													<logic:iterate id="format" name="SupervisionFormatter" type="ieci.tdw.ispac.ispaclib.bean.BeanPropertyFmt">
														<logic:equal name="format" property="property" value="RESPNAME">
															<%
																String org = _showResponsiblesForSupervision + "&supervDirView=organization";
																org += "&dirUid=" + orgunit.getProperty("UID");
															%>
															<html:link href='<%= org %>' styleClass="resplink">
					...
					<tr>
						<td>
							...
							<div class="stdform">
								...
								<html:form action='<%=_addSuperviseds + "&supervDirView=groups"%>'>
									...
									<table class="tableDisplayUserManager">
										...
										<logic:present name="supervGroups">
											<logic:iterate id="group" name="supervGroups" type="ieci.tdw.ispac.ispaclib.bean.ItemBean">
												<tr>
													<td width='17px'>
														<logic:empty name="group" property="property(SELECTED)">
															<html:multibox property="uids">
													...
													<td colspan='2' align='left' class='ldapentry' title='<%= group.getProperty("RESPNAME") %>'>
														<logic:iterate id="format" name="SupervisionFormatter" type="ieci.tdw.ispac.ispaclib.bean.BeanPropertyFmt">
															<logic:equal name="format" property="property" value="RESPNAME">
																<%
																	String grp = _showResponsiblesForSupervision + "&supervDirView=groups";
