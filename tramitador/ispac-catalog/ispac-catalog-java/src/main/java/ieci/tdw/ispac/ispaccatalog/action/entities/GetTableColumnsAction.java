...
public class GetTableColumnsAction extends AjaxSuggestAction{
	...
	public List getLabelValuedBeansList(SessionAPI session) throws ISPACException {
		...
		DbResultSet resultado = cct.getConnection().getColumns(getExtraParam(), getFilter());
		...
		try {
			...
			while(rs.next()){
				...
				if(getNumRows() > -1){
					 if (++contador > getNumRows()){
						 ...
						 aux.add(new LabelValueBean(rs.getString("COLUMN_NAME"), rs.getString("COLUMN_NAME")));
					 }
				}else{
					aux.add(new LabelValueBean(rs.getString("COLUMN_NAME"), rs.getString("COLUMN_NAME")));
				}
			...
			throw new ISPACException("Error recuperando campos de la tabla.");
		}finally{
