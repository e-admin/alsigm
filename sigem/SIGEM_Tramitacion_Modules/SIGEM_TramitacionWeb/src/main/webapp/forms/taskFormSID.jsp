	...
	function deleteDocument() {
		...
		if (data != "") {
			jConfirm('<bean:message key="ispac.action.documents.delete"/>', '<bean:message key="common.confirm"/>', '<bean:message key="common.message.ok"/>' , '<bean:message key="common.message.cancel"/>', function(r) {
				if(r){
					document.defaultForm.action = document.defaultForm.action + "?taskId=" + '<%=request.getParameter("taskId")%>' + "&actions=deleteDocument";
					document.defaultForm.submit();
				}
			});
		} else {
												...
												<tr>
													<td>
														<table cellspacing="0" cellpadding="0" align="center" width="90%">
															...
															<tr>
																<td height="10px">
																	<ispac:linkframe 	id="ServicioIntermediacion"
																			target="workframe"
																			action='/apps/redireccionarCertificados.jsp'
																			titleKey="svd.servicios.search"
																			showFrame="true"
																			inputField=""
																			styleClass="tdlink"
																			width="500"
