...
public class SecretariaResourcesHelper {

	/** Nombre del fichero de recursos. */
	private static String BUNDLE_NAME = "ieci.tecdoc.sgm.tram.secretaria.resources.SecretariaMessages";
	...
	public static void setInfoMessage(IRuleContext rulectx,
		...
		if (stateContext != null) {
			...
			if (currentState.getClass().equals(stateClass)) {
				...
				rulectx.setInfoMessage(SecretariaResourcesHelper.getMessage(cct.getLocale(), stateKeyMessage, stateArgs));
				return;
			}
		}

		rulectx.setInfoMessage(SecretariaResourcesHelper.getMessage(cct.getLocale(), noStateKeyMessage, noStateArgs));
	}
