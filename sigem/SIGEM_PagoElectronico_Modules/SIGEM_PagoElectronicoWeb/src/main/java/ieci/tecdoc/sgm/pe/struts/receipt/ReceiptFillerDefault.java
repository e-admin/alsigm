...
public class ReceiptFillerDefault implements ReceiptFiller{
	...
	private static final String XPATH_TIPO_TASA="TIPO_TASA";
...
//		}
		...
		if(elem!=null) poFormFlds.setField(FIELD_IDENTIFICADOR2, elem.getValue());
		...
		elem = tasa.getChildElement(XPATH_TIPO_TASA);
		String tipoTasa=elem.getValue();
		
		elem = tasa.getChildElement(XPATH_MODELO_TASA);
		String modeloTasa=elem.getValue();
		
		elem = tasa.getChildElement(XPATH_NOMBRE_TASA);
		poFormFlds.setField(FIELD_NOMBRE_TASA, elem.getValue());
		
		if(StringUtils.isEmpty(modeloTasa)) poFormFlds.setField(FIELD_MODELO, modeloTasa);
	}
}
