...
import ieci.tecdoc.sgm.admsistema.proceso.exportacion.Exportar;
...
public class ExportarEntidadAction extends AdministracionWebAction {
	private static final Logger logger = Logger.getLogger(ExportarEntidadAction.class);
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (baseDatosForm == null || Utilidades.esNuloOVacio(baseDatosForm.getIdEntidad())) {
				...
				if (entidad == null) {
					request.setAttribute(Defs.MENSAJE_ERROR, "mensaje.error.proceso.exportar.error_inesperado");
				...
				if (Utilidades.permisoAdministrarEntidad(usuario, entidad.getIdentificador())) {
					...
					logger.debug("El usuario " + usuario + " ha intentado exportar la entidad " + entidad.getIdentificador() + " sin tener permisos");
					request.setAttribute(Defs.MENSAJE_ERROR, "mensaje.error.proceso.exportar.error_sin_permiso");
					return mapping.findForward(FORWARD_ERROR);
				}
			} else {
				if (form != null) {
...
//					};
					...
					if (Exportar.exportar(parametros)) {
						request.setAttribute(Defs.MENSAJE_INFORMATIVO, "mensaje.informativo.proceso.exportar.correcto");
						return mapping.findForward(FORWARD_SUCCESS);
					} else {
						request.setAttribute(Defs.MENSAJE_ERROR, "mensaje.error.proceso.exportar.incorrecto");
						return mapping.findForward(FORWARD_ERROR);
					}
				}
			}
