		...
		<SCRIPT language=javascript>
			function enviarFormulario() {
				if (validateFormulario()){
					...
					ocultarMensajes();
					mostrarMensaje (true, "<bean:message key="acciones.multientidad.mensaje.error.opcion.seleccione" />");
				}
			}

			function validateFormulario() {
				var radios = document.accionMultientidadBean.checkOpcion;
				if (radios != null) {
					if (radios.length) {
		...
		<div id=contenedora align=center>
			<div class=cuerpo style="width: 80%">
				<div class=cuerporight>
					<div class=cuerpomid>
						<h1><bean:message key="acciones.multientidad.seleccion.opcion.titulo"/></h1>
						<div class=submenu3>
							<ul>
  								<li class=submen1on>
						...
						<div class=cuadro>
							<div>
								...
								<html:form styleId="formulario" action="/ejecutarConfiguracionAccionMultientidad.do" method="post">
									...
									<display-el:table name="${accionMultientidadBean.opcionesConfiguracion}"
										uid="opcion"
										export="false"
										...
										<display:column  style="width: 20px; height: 100%" media="html" titleKey="cabeceraTabla.entidades.panelBotones" sortable="false" headerClass="cabeceraTabla" >
											<html-el:radio styleId="checkOpcion" property="opcion" title="${opcion.label}" value="${opcion.id}" style="border: 0px; width: 20px"></html-el:radio>
										</display:column>
										<display:column  style="height: 100%" media="html" titleKey="acciones.multientidad.opcion" sortable="true" headerClass="cabeceraTabla" >
											<bean:write name="opcion" property="label" />
										</display:column>
									</display-el:table>
								</html:form>
							</div>
